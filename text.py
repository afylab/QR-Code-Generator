...
DEFAULT_FACE = freetype.Face('C:\Software\snakenbake-master\Vera.ttf')
...
def bezier(points, t):
    ...
    n = len(points) - 1
    return (bernstein(n, np.arange(n+1)[:,np.newaxis], t)[...,np.newaxis] * points[:,np.newaxis]).sum(axis=0)
...
def render_character(char, face=DEFAULT_FACE, points_per_segment=POINTS_PER_SEGMENT):
    ...
    for start, end in zip(np.concatenate(((0,), contours[:-1]+1)), contours+1):
        ...
        for j in range(len(points)):
            if (tags[j] & freetype.FT_CURVE_TAG_CONIC
                and j > 1 and tags[j-1] & freetype.FT_CURVE_TAG_CONIC):
        ...
        for segment in segments:
            if len(segment) == 2:
                ...
                polygon_segments.append(bezier(np.array(segment), np.linspace(0, 1, points_per_segment))[:-1])
            else:
                raise ValueError('unexpected number of points per segment {}'.format(len(segment)))
        polygon = np.concatenate(polygon_segments)
        polygons.append(polygon)
    metrics = {attr: getattr(face.glyph.metrics, attr) for attr in ('vertAdvance', 'horiAdvance')}
...
def cut_out_hole(exterior, interior, layer=0, datatype=0):
    ...
    #                               interior[idx_int::-1],
    #                               exterior[idx_ext:]))
    result = np.concatenate((exterior[:idx_ext+1],
                             interior[idx_int:],
                             interior[:idx_int+1],
                             exterior[idx_ext:]))
    return result
...
def cut_out_holes(polygons, layer=0, datatype=0):
    ...
    for polygon_ext in polygons:
        ...
        if len(polygons_to_cut) >= 2:
            used_polygons.add(make_hashable(polygons_to_cut[0])) # don't add an un-cut exterior
    ...
    result = [reduce(partial(cut_out_hole, layer=layer, datatype=datatype), polygons_to_cut) for polygons_to_cut in top_level_polygons]
    return g.PolygonSet(result, layer=layer, datatype=datatype)

@memoize
def character(char, face=DEFAULT_FACE, scale_factor=1, points_per_segment=POINTS_PER_SEGMENT,
              layer=0, datatype=0):
    cell_name = '~{}~l{}'.format(char, layer)
    if scale_factor != 1:
        cell_name += 's{}'.format(scale_factor)
    cell = Cell(cell_name)
    polygons, metrics = render_character(char, face=face, points_per_segment=points_per_segment)
    polygons = [p * scale_factor for p in polygons]
    cell.add(cut_out_holes(polygons, layer=layer, datatype=datatype))
    return cell, metrics

def Text(text, size, position=(0, 0), horizontal=True, angle=0,
          layer=0, datatype=0, face=DEFAULT_FACE,
          scale_factor=1, points_per_segment=POINTS_PER_SEGMENT):
    ...
    for char in text:
        if char == "\n":
            ...
            continue
        char_cell, metrics = character(char, face=face, scale_factor=scale_factor, points_per_segment=points_per_segment,
                                       layer=layer, datatype=datatype)
        cells.append(g.CellReference(char_cell, position + offset, rotation=0, magnification=magnification))
